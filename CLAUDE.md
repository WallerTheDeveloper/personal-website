...
- The Projects panel has one sub-route, `#projects/pN`. It is a **second axis beside `current`, not a fifth destination**: it never starts a jump, so `jump()`, `JumpGuard`, `_pending` and `_going` are not involved in changing it. `openDetail()`/`closeDetail()` push and then drive the view directly, exactly as `go()` does — they exist *because* `go()` early-returns on `id === current` and a detail always opens over a panel that is already current. Never route one through `go()`.
...
1. The Projects panel carries the visible notice “Independent projects - work built on my own initiative.” The panel is named *Independent Projects* everywhere — `<h1>`, `aria-label`, `panel__where`, the hero label and `TITLES.projects`, matching the prototype. An earlier revision named it *Freelance Projects* on the reading that the panel held client work; it does not, and that rename was reverted. Nothing on this site is client work: the XR panel's `Independent · …` card is the owner's own project too, and the ZAUBAR block is the only employment anywhere.
